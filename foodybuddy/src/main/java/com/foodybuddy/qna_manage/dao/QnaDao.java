...
public class QnaDao {
	public List<Qna> getAllQna(Connection conn) {
        ...
        String query = "SELECT qna_no, user_no, qna_title, qna_content, reg_date, qna_status, qna_answer, mod_time, complete_date FROM user_QnA";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Timestamp modTimestamp = rs.getTimestamp("mod_time");
                qna.setmod_time(modTimestamp);
    ...
    public List<Qna> getQnaByStatus(Connection conn, String status) {
        List<Qna> qnaList = new ArrayList<>();
        String query = "SELECT qna_no, user_no, qna_title, qna_content, reg_date, qna_status, qna_answer, mod_time, complete_date FROM user_QnA WHERE qna_status = ?";
        PreparedStatement pstmt = null;
        ResultSet rs = null;
    ...
    public Qna getQnaByNo(Connection conn, int qnaNo) {
        Qna qna = null;
        String query = "SELECT qna_no, user_no, qna_title, qna_content, reg_date, qna_status, qna_answer, mod_time, complete_date FROM user_QnA WHERE qna_no = ?";
        PreparedStatement pstmt = null;
        ResultSet rs = null;
    ...
    public int updateQnaAnswer(Connection conn, int qnaNo, String answer) {
        String query = "UPDATE user_QnA SET qna_answer = ?, qna_status = '완료', mod_time = CURRENT_TIMESTAMP, complete_date = CURRENT_TIMESTAMP WHERE qna_no = ?";
        PreparedStatement pstmt = null;
        int result = 0;
